"use client";

import { useEffect, useState } from "react";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { ExternalLink, ShoppingBag } from "lucide-react";
import { useAuthStore } from "@/lib/store/useAuthStore";
import { toast } from "@/hooks/use-toast";

interface Product {
  id: string;
  title: string;
  brand: string;
  category: string;
  price: number;
  imageUrl: string;
  url: string;
}

export function StorePage() {
  const { user } = useAuthStore();
  const [products, setProducts] = useState<Product[]>([]);

  useEffect(() => {
    loadProducts();
  }, []);

  const loadProducts = async () => {
    // Mock data - replace with actual API call
    const mockProducts: Product[] = [
      {
        id: "1",
        title: "Premium Whey Protein",
        brand: "ProSupps",
        category: "Supplements",
        price: 49.99,
        imageUrl: "https://images.unsplash.com/photo-1593095948071-474c5cc2989d?w=300&h=300&fit=crop",
        url: "https://example.com/protein"
      },
      {
        id: "2",
        title: "Lifting Straps Pro",
        brand: "IronGrip",
        category: "Equipment",
        price: 24.99,
        imageUrl: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=300&h=300&fit=crop",
        url: "https://example.com/straps"
      },
      {
        id: "3",
        title: "Resistance Bands Set",
        brand: "FitLoop",
        category: "Equipment",
        price: 34.99,
        imageUrl: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=300&h=300&fit=crop",
        url: "https://example.com/bands"
      },
      {
        id: "4",
        title: "Pre-Workout Energy",
        brand: "BeastMode",
        category: "Supplements",
        price: 39.99,
        imageUrl: "https://images.unsplash.com/photo-1593095948071-474c5cc2989d?w=300&h=300&fit=crop",
        url: "https://example.com/preworkout"
      }
    ];

    setProducts(mockProducts);
  };

  const handleProductClick = (product: Product) => {
    toast({
      title: "Opening product",
      description: "Redirecting to affiliate store..."
    });
    
    // Track affiliate click (mock)
    console.log('Affiliate click tracked:', product.id, user?.id);
    
    // Open product URL
    window.open(product.url, '_blank');
  };

  const handleUpgradeMembership = () => {
    toast({
      title: "Upgrade Membership",
      description: "Redirecting to Whop membership upgrade..."
    });
  };

  return (
    <div className="max-w-7xl mx-auto space-y-8">
      {/* Header */}
      <div className="space-y-2">
        <h1 className="text-3xl font-bold">Store</h1>
        <p className="text-muted-foreground">Shop our recommended products and gear</p>
      </div>

      {/* Member Discount Banner */}
      {!user?.isMember && (
        <Card className="p-6 bg-gradient-to-br from-purple-500/5 to-blue-500/5 border-purple-500/20">
          <div className="text-center space-y-4">
            <h3 className="text-xl font-semibold">Unlock Exclusive Discounts</h3>
            <p className="text-muted-foreground">
              Members get special pricing on all store items. Upgrade your membership for access.
            </p>
            <Button onClick={handleUpgradeMembership} className="gap-2">
              Upgrade via Whop
            </Button>
          </div>
        </Card>
      )}

      {/* Products Grid */}
      {products.length === 0 ? (
        <div className="text-center py-12">
          <ShoppingBag className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
          <h3 className="text-lg font-semibold mb-2">No products available</h3>
          <p className="text-muted-foreground">Check back soon for our curated product selection</p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {products.map(product => (
            <Card 
              key={product.id} 
              className="overflow-hidden hover:shadow-lg transition-all cursor-pointer hover:scale-105"
              onClick={() => handleProductClick(product)}
            >
              <div className="relative">
                <img 
                  src={product.imageUrl} 
                  alt={product.title}
                  className="w-full h-48 object-cover"
                />
                {user?.isMember && (
                  <Badge className="absolute top-2 right-2 bg-purple-500">
                    Member Price
                  </Badge>
                )}
              </div>
              
              <div className="p-4 space-y-3">
                <div className="space-y-1">
                  <Badge variant="secondary" className="text-xs">{product.category}</Badge>
                  <h3 className="font-semibold line-clamp-2">{product.title}</h3>
                  <p className="text-sm text-muted-foreground">{product.brand}</p>
                </div>
                
                <div className="flex items-center justify-between pt-2 border-t">
                  <span className="text-lg font-bold">${product.price}</span>
                  <Button size="sm" variant="outline" className="gap-2">
                    Shop
                    <ExternalLink className="h-3 w-3" />
                  </Button>
                </div>
              </div>
            </Card>
          ))}
        </div>
      )}
    </div>
  );
}
